...
const typeIntoTextBox = (textboxName: string, value: string) => {
  const element = screen.getByRole("textbox", {name:textboxName});
  userEvent.type(element, value);
  return element;
}
...
export { changeInputValueByEl, changeInputByTestId, getInputFieldByTestId, getInputFieldsByTestId, typeIntoTextBox, typeIntoLabelField };
